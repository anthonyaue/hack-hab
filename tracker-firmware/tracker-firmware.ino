...
void setup()
{
  ...
  buzzer_setup();
  Serial.println("Setting up afsk");
  afsk_setup();
  Serial.println("Setting up afsk");
  gps_setup();
  ...
  if (APRS_SLOT >= 0) {
    Serial.println("APRS setup with slot (waiting for time ref)");
    do {
    ...
    Serial.println("No APRS_SLOT, just querying millis.");
    next_aprs = millis();
  }  
...
void get_pos()
{
  ...
  if (valid_pos) {
    Serial.println("Valid position acquired\n");
